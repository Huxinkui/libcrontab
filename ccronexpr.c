...
static int do_next(cron_expr* expr, struct tm* calendar, unsigned int dot) {
    ...
    update_year = find_next(expr->years, CRON_MAX_YEARS, year, calendar, CRON_CF_YEAR, -1, resets, &res);
    if (0 != res) goto return_result;
    if (year == update_year) {
        push_to_fields_arr(resets, CRON_CF_YEAR);
    } else {
        res = do_next(expr, calendar, dot);
        if (0 != res) goto return_result;
    }
...
void cron_parse_expr(const char* expression, cron_expr* target, const char** error) {
    ...
    *error = NULL;
    ...
    if (*error) goto return_res;
    set_number_hits(fields[6], target->years, 0, 200, error);
    if (*error) goto return_res;
...
static int do_prev(cron_expr* expr, struct tm* calendar, unsigned int dot) {
    ...
    year = calendar->tm_year;
    update_year = find_prev(expr->years, CRON_MAX_YEARS, year, calendar, CRON_CF_YEAR, -1, resets, &res);
    if (0 != res) goto return_result;
    if (year == update_year) {
        push_to_fields_arr(resets, CRON_CF_YEAR);
    } else {
